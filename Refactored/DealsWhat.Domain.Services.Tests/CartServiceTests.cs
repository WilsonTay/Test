...
namespace DealsWhat.Domain.Services.Tests
{
    ...
    public class CartServiceTests
    {
        ...
        public void AddCartItem_GetUserFromRepository()
        {
            ...
            mockedDealRepository.Setup(m => m.FindByKey(It.IsAny<string>()))
                .Returns(deal);
        ...
        public void AddCartItem_AddToUserCart()
        {
            ...
            userCartItem.Deal.ShouldBeEquivalentTo(deal);
        ...
        [TestMethod]
        public void GetCartItems_AccessUserCartsFromRepository2()
        {
            var user = TestModelFactory.CreateUser();

            var deal = TestModelFactory.CreateCompleteDeal();
            var dealOption = deal.Options.First();
            var dealAttributeGroups = dealOption.Attributes.GroupBy(a => a.Name);

            // Group by Color, Size etc.
            var dealAttributes = new List<DealAttributeModel>();
            foreach (var group in dealAttributeGroups)
            {
                dealAttributes.Add(group.First());
            }

            var cartItem = TestModelFactory.CreateCartItem(deal, dealOption, dealAttributes);
            ...
            var actualCartItem = cartItems.First();

            actualCartItem.ShouldBeEquivalentTo(cartItem);
            actualCartItem.Deal.ShouldBeEquivalentTo(deal);
            actualCartItem.DealOption.ShouldBeEquivalentTo(dealOption);
            actualCartItem.DealOption.Attributes.ShouldAllBeEquivalentTo(dealAttributes);
        }
